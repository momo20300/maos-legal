    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm bg-card border border-border rounded-xl shadow-lg p-8 flex flex-col items-center">
          ...
          <p className="text-muted-foreground text-sm mb-6">{t.auth.signUpSubtitle}</p>

          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
            {/* Name row */}
            <div className="flex gap-3">
              <div className="flex flex-col gap-1.5 flex-1">
                <label className="text-sm font-medium text-foreground">{t.auth.firstNameLabel}</label>
                <input
                  type="text"
                  value={firstName}
                  onChange={(e) => setFirstName(e.target.value)}
                  placeholder="Jean"
                  className="w-full px-3 py-3 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-accent text-sm"
                />
              </div>
              <div className="flex flex-col gap-1.5 flex-1">
                <label className="text-sm font-medium text-foreground">{t.auth.lastNameLabel}</label>
                <input
                  type="text"
                  value={lastName}
